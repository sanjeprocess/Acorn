...
export const uploadDocs = asyncHandler(async (req, res) => {
  ...
  const normalizeField = (name = "") => name.replace(/\[\]$/, "").trim();
  const sanitizeFileName = (name = "file.pdf") =>
    name
      .replace(/[^a-zA-Z0-9._-]+/g, "_")
      .replace(/_+/g, "_")
      .replace(/^_+|_+$/g, "") || "file.pdf";
  const debugReceivedFields = [];

  // Supports both multer.any() (array) and multer.fields() (object map).
  const filesByField = {};
  if (Array.isArray(req.files)) {
    for (const file of req.files) {
      const key = normalizeField(file.fieldname);
      debugReceivedFields.push(`${file.fieldname}:${file.originalname}`);
    ...
    for (const [fieldName, files] of Object.entries(req.files)) {
      ...
      for (const file of list) {
        debugReceivedFields.push(`${fieldName}:${file.originalname}`);
        filesByField[key].push(file);
      }
    }
  }
  ...
  const existingOtherDocs =
    travel.otherDocs && typeof travel.otherDocs.toObject === "function"
      ? travel.otherDocs.toObject()
      : travel.otherDocs || {};

  const docFields = [
    ...new Set([
      ...baseDocFields,
      ...Object.keys(existingOtherDocs),
      ...Object.keys(filesByField),
    ]),
  ];

  for (const field of docFields) {
    const existingFiles = Array.isArray(existingOtherDocs[field])
      ? existingOtherDocs[field]
      : [];
    const newFiles = filesByField[field] || [];
    ...
    const uploadedUrls = await Promise.all(
      newFiles.map((file, index) =>
        uploadFileToFirebase(
          file,
          `travels/${field}-${Date.now()}-${index}-${sanitizeFileName(file.originalname)}`
        )
      )
    );
  ...
  travel.otherDocs = { ...existingOtherDocs, ...uploadedResults };
  ...
  res.json({
    ...
    success: true,
    debug: {
      receivedFields: debugReceivedFields,
      mappedFields: Object.keys(filesByField),
      counts: Object.fromEntries(
        docFields.map((field) => [field, (filesByField[field] || []).length])
      ),
    },
  });
});
...
export const deleteCustomerDoc = asyncHandler(async (req, res) => {
  ...
  const existingOtherDocs =
    travel.otherDocs && typeof travel.otherDocs.toObject === "function"
      ? travel.otherDocs.toObject()
      : travel.otherDocs || {};
  const travelFieldDocs = Array.isArray(existingOtherDocs[field])
    ? existingOtherDocs[field]
    : [];
